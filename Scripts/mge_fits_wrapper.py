...
import mge_fit_Hoyer22_data as m22
...
phys_ext = phys_ext+'_extinction_corr_nsa_ml_w_vi_color'
...
gal_table_filename = '../Data_Sets/Lauer2005_data/gal_properties_Lauer_2005.fit'
...
# remove the duplicates found in Pechetti+17
names_05 = names_05[np.where(names_05 != 'NGC 2778')] 
names_05 = names_05[np.where(names_05 != 'NGC 4458')] 
names_05 = names_05[np.where(names_05 != 'NGC 4660')] 
names_05 = names_05[np.where(names_05 != 'NGC 7457')]

# remove 3 gals with no mass/color in David's table
names_05 = names_05[np.where(names_05 != 'NGC 1700')] 
names_05 = names_05[np.where(names_05 != 'NGC 4709')] 
names_05 = names_05[np.where(names_05 != 'NGC 7785')] 
...
MGEs_05 = np.zeros((100,len(names_05)))
g_heights_05 = []
g_sigs_05 = []
SBs_interp_converted_05 = np.zeros((100,len(names_05)))
SBs_true_converted_05 = []
all_MLs_05 = np.zeros((len(names_05),6)) - 99
all_ML_types_05 = np.zeros((len(names_05),6)) - 99
gicolors_05 = np.zeros(len(names_05))
dist_flags_05 = np.zeros(len(names_05))
filts_05 = np.zeros(len(names_05)).astype(str)
for i in tqdm(range(len(names_05)), position=0, leave=True):
    ...
    dens_profiles_05[:,i], radii_05[:,i], lums_05[i], MLs_05[i], \
        ...
        true_SB_converted, all_MLs, \
        all_ML_types, gicolors_05[i], dist_flags_05[i], MGEs_05[:,i], filts_05[i] = m05.get_density_profile(s[0], s[1],max_rad)
    for k in range(len(all_MLs)):
        all_MLs_05[i,k] = float(all_MLs[k])
        all_ML_types_05[i,k] = int(all_ML_types[k])
...
SBs_true_05 = np.array(SBs_true_05)
rad_true_05 = np.array(rad_true_05)
g_heights_05 = np.array(g_heights_05)
g_sigs_05 = np.array(g_sigs_05)
SBs_true_converted_05 = np.array(SBs_true_converted_05)

#pdb.set_trace()
# let's remove the galaxies here that have minimum radius > 5 pc
keepi = np.where(radii_05[0,:] <= 5)
names_05 = names_05[keepi[0]]
dens_profiles_05 = dens_profiles_05[:,keepi[0]]
radii_05 = radii_05[:,keepi[0]]
lums_05 = lums_05[keepi[0]]
ML_types_05 = ML_types_05[keepi[0]]
MLs_05 = MLs_05[keepi[0]]
dists_05 = dists_05[keepi[0]]
ras_05 = ras_05[keepi[0]]
decs_05 = decs_05[keepi[0]]
SBs_true_05 = SBs_true_05[keepi[0]]
rad_true_05 = rad_true_05[keepi[0]]
SBs_interp_05 = SBs_interp_05[:,keepi[0]]
rad_interp_05 = rad_interp_05[:,keepi[0]]
g_heights_05 = g_heights_05[keepi[0]]
g_sigs_05 = g_sigs_05[keepi[0]]
SBs_interp_converted_05 = SBs_interp_converted_05[:,keepi[0]]
SBs_true_converted_05 = SBs_true_converted_05[keepi[0]]
all_MLs_05 = all_MLs_05[keepi[0],:]
all_ML_types_05 = all_ML_types_05[keepi[0],:]
gicolors_05 = gicolors_05[keepi[0]]
dist_flags_05 = dist_flags_05[keepi[0]]
MGEs_05 = MGEs_05[:,keepi[0]]
filts_05 = filts_05[keepi[0]]
...
# 
# # remove the duplicates found in Pechetti+17
# names_95 = names_95[np.where(names_95 != 'n4387')]
# names_95 = names_95[np.where(names_95 != 'n4434')]
# names_95 = names_95[np.where(names_95 != 'n4551')]
# 
# =============================================================================
# we have removed all but 6 galaxies from this sample so manually set lauer+95 name
names_95 = np.array(['v1440','n4464','v1545','n4467','v1627','v1199'])
...
MGEs_95 = np.zeros((100,len(names_95)))
g_heights_95 = []
g_sigs_95 = []
SBs_interp_converted_95 = np.zeros((100,len(names_95)))
SBs_true_converted_95 = []
all_MLs_95 = np.zeros((len(names_95),6)) - 99
all_ML_types_95 = np.zeros((len(names_95),6)) - 99
gicolors_95 = np.zeros(len(names_95))
dist_flags_95 = np.zeros(len(names_95))
filts_95 = np.zeros(len(names_95)).astype(str)
for i in tqdm(range(len(names_95)), position=0, leave=True):
    ...
    dens_profiles_95[:,i], radii_95[:,i], lums_95[i], \
        ...
        true_SB_converted, all_MLs, all_ML_types, gicolors_95[i], \
        dist_flags_95[i], MGEs_95[:,i], filts_95[i] = m95.get_density_profile(names_95[i],max_rad)
    
    for k in range(len(all_MLs)):
        all_MLs_95[i,k] = float(all_MLs[k])
        all_ML_types_95[i,k] = int(all_ML_types[k])
...
# below isn't necessary since the names have been manually edited to only include these
# =============================================================================
# # let's remove the galaxies here that have minimum radius > 5 pc
# names_95 = names_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# dens_profiles_95 = dens_profiles_95[:,np.where(rad_interp_95[:,0] <= np.log10(5))]
# radii_95 = radii_95[:,np.where(rad_interp_95[:,0] <= np.log10(5))]
# lums_95 = lums_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# ML_types_95 = ML_types_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# MLs_95 = MLs_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# dists_95 = dists_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# ras_95 = ras_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# decs_95 = decs_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# SBs_true_95 = SBs_true_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# rad_true_95 = rad_true_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# SBs_interp_95 = SBs_interp_95[:,np.where(rad_interp_95[:,0] <= np.log10(5))]
# rad_interp_95 = rad_interp_95[:,np.where(rad_interp_95[:,0] <= np.log10(5))]
# g_heights_95 = g_heights_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# g_sigs_95 = g_sigs_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# SBs_interp_converted_95 = SBs_interp_converted_95[:,np.where(rad_interp_95[:,0] <= np.log10(5))]
# SBs_true_converted_95 = SBs_true_converted_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# all_MLs_95 = all_MLs_95[np.where(rad_interp_95[:,0] <= np.log10(5)),:]
# all_ML_types_95 = all_ML_types_95[np.where(rad_interp_95[:,0] <= np.log10(5)),:]
# gicolors_95 = gicolors_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# dist_flags_95 = dist_flags_95[np.where(rad_interp_95[:,0] <= np.log10(5))]
# =============================================================================
...
names_17 = names_17[np.where(names_17 != 'NGC4339')] # remove for lack of nucler color info
names_17 = names_17[np.where(names_17 != 'NGC2699')] # remove for lack of nucler color info
names_17 = names_17[np.where(names_17 != 'NGC2549')] # remove for lack of nucler color info
...
g_heights_17 = []
g_sigs_17 = []
rad_17 = np.zeros((100,len(names_17)))
SBs_17 = np.zeros((100,len(names_17)))
MGEs_17 = np.zeros((100,len(names_17)))
all_MLs_17 = np.zeros((len(names_17),6)) - 99
all_ML_types_17 = np.zeros((len(names_17),6)) - 99
gicolors_17 = np.zeros(len(names_17))
dist_flags_17 = np.zeros(len(names_17))
for i in tqdm(range(len(names_17)), position=0, leave=True):
    ...
    dens_profiles_17[:,i], radii_17[:,i], lums_17[i], \
        ...
        decs_17[i], filts_17[i], heights, sigs, rad_17[:,i], SBs_17[:,i],\
        all_MLs, all_ML_types, gicolors_17[i], MGEs_17[:,i] = mp17.get_density_profile(names_17[i])
    
    for k in range(len(all_MLs)):
        all_MLs_17[i,k] = float(all_MLs[k])
        all_ML_types_17[i,k] = int(all_ML_types[k])
        
    g_heights_17.append(heights)
    g_sigs_17.append(sigs)
    sys.stdout = old_stdout
    #pdb.set_trace()
...
names_22 = np.unique(names_22)

names_22 = names_22[np.where(names_22 != 'eso553-046')]

#pdb.set_trace()
dens_profiles_22 = np.zeros((100,len(names_22)))
radii_22 = np.zeros((100,len(names_22)))
lums_22 = np.zeros(len(names_22))
ML_types_22 = np.zeros(len(names_22))
MLs_22 = np.zeros(len(names_22))
dists_22 = np.zeros(len(names_22))
ras_22 = np.zeros(len(names_22))
decs_22 = np.zeros(len(names_22))
filts_22 = np.zeros(len(names_22)).astype(str)
pixel_scales_22 = np.zeros(len(names_22))
g_heights_22 = []
g_sigs_22 = []
SBs_22 = np.zeros((100,len(names_22)))
rad_22 = np.zeros((100,len(names_22)))
MGEs_22 = np.zeros((100,len(names_22)))
all_MLs_22 = np.zeros((len(names_22),6)) - 99
all_ML_types_22 = np.zeros((len(names_22),6)) - 99
gicolors_22 = np.zeros(len(names_22))
dist_flags_22 = np.zeros(len(names_22))
for i in tqdm(range(len(names_22)), position=0, leave=True):
    sys.stdout = open(os.devnull, "w")
    dens_profiles_22[:,i], radii_22[:,i], lums_22[i], \
         MLs_22[i], ML_types_22[i], dists_22[i], ras_22[i], \
         decs_22[i], filts_22[i], pixel_scales_22[i], \
         heights, sigs, SBs_22[:,i], rad_22[:,i], \
         all_MLs, all_ML_types, gicolors_22[i], MGEs_22[:,i] = m22.get_density_profile(names_22[i])
    
    for k in range(len(all_MLs)):
        all_MLs_22[i,k] = float(all_MLs[k])
        all_ML_types_22[i,k] = int(all_ML_types[k])
        
    g_heights_22.append(heights)
    g_sigs_22.append(sigs)
    sys.stdout = old_stdout
    #pdb.set_trace()
...
for i in range(len(names_05)):
    ...
    all_SBrads.append(SBs_interp_converted_05[:,i])
    all_SBs.append(rad_interp_05[:,i])
    all_MGEs.append(MGEs_05[:,i])
    ls.append(a)
# add lauer95 profiles to list
for i in range(len(names_95)):
    a = np.zeros((len(radii_95[:,0]),2))
    ...
    all_SBrads.append(SBs_interp_converted_95[:,i])
    all_SBs.append(rad_interp_95[:,i])
    all_MGEs.append(MGEs_95[:,i])
    ls.append(a)
...
for i in range(len(names_17)):
    ...
    all_SBrads.append(SBs_17[:,i])
    all_SBs.append(rad_17[:,i])
    all_MGEs.append(MGEs_17[:,i])
    ls.append(a)
# add hoyer22 profiles to list
for i in range(len(names_22)):
    a = np.zeros((len(radii_22[:,2]),2))
    a[:,0] = np.log10(radii_22[:,i])
    a[:,1] = np.log10(dens_profiles_22[:,i])
    all_lograds.append(np.log10(radii_22[:,i]))
    all_logdens.append(np.log10(dens_profiles_22[:,i]))
    all_SBrads.append(rad_22[:,i])
    all_SBs.append(SBs_22[:,i])
    all_MGEs.append(MGEs_22[:,i])
    ls.append(a)

# concatenate all the v-band magnitudes for colorbar
lumys = np.concatenate((lums_05,lums_95,lums_17,lums_22))
...
hst_ps_17 = (0.0456*arcsec_to_radian)*dists_17
hst_ps_22 = (pixel_scales_22*arcsec_to_radian)*dists_22 # assuming F814W filter is used

all_hst_ps = np.concatenate((hst_ps_05,hst_ps_95,hst_ps_17,hst_ps_22))
...
slopes_17 = slopes[len(names_05)+len(names_95):len(names_05)+len(names_95)+len(names_17)]
slopes_22 = slopes[len(names_05)+len(names_95)+len(names_17):]

central_dens_05 = central_dens[0:len(names_05)]
central_dens_95 = central_dens[len(names_05):len(names_05)+len(names_95)]
central_dens_17 = central_dens[len(names_05)+len(names_95):len(names_05)+len(names_95)+len(names_17)]
central_dens_22 = central_dens[len(names_05)+len(names_95)+len(names_17):]
...
for i in range(len(names_17)):
    s1 = names_17[i][0:3]
    s2 = names_17[i][3:]
    names_17_c.append(s1+' '+s2)
names_17_c = np.array(names_17_c)

#%%    
all_names = np.concatenate((names_05_c,names_95_c,names_17_c,names_22))
...
data05_3 = ascii.read(table='../Data_Sets/Lauer05_Table3.txt', format='aastex') 
    ...
    for j in range(len(data05_3['Gal Name'])):
        if data05_3['Gal Name'][j] == all_names[i]:
            ...
            if data05_3['Nuc'][j] == '$\\times$':
                ...
                NSC_comp[i] = 0
    if not found:
        if all_names[i] in names_22:
            NSC_comp[i] = 1 # all Hoyer+22 gals are nucleated
        else:    
            NSC_comp[i] = -1
...
all_dists = np.concatenate((dists_05,dists_95,dists_17,dists_22))
all_MLs = np.concatenate((MLs_05,MLs_95,MLs_17,MLs_22))
all_ML_types = np.concatenate((ML_types_05,ML_types_95, ML_types_17,ML_types_22))
all_ras = np.concatenate((ras_05,ras_95,ras_17,ras_22))
all_decs = np.concatenate((decs_05,decs_95,decs_17,decs_22))
all_gicolors = np.concatenate((gicolors_05, gicolors_95, gicolors_17, gicolors_22))
all_dist_flags = np.concatenate((dist_flags_05,dist_flags_95,dist_flags_17,dist_flags_22))
all_filts = np.concatenate((filts_05,filts_95,filts_17,filts_22))

all_MLs_mult = np.concatenate((all_MLs_05,all_MLs_95,all_MLs_17,all_MLs_22))
all_ML_types_mult = np.concatenate((all_ML_types_05,all_ML_types_95,all_ML_types_17,all_ML_types_22))

all_MLs_es = []
all_ML_types_es = []
for i in range(len(all_MLs_mult[:,0])):
    all_MLs_es.append(all_MLs_mult[i,:])
    all_ML_types_es.append(all_ML_types_mult[i,:])
...
c15 = fits.Column(name='all_mls', array=all_MLs_es, format='6D')
c16 = fits.Column(name='all_ml_types', array=all_ML_types_es, format='6I')
c17 = fits.Column(name='gicolor', array=all_gicolors, format='D')
c18 = fits.Column(name='dist_flag', array=all_dist_flags, format='I')
c19 = fits.Column(name='SBs', array=all_SBs, format='100D', unit='L_sol/pc^3')
c20 = fits.Column(name='MGEs', array=all_MGEs, format='100D', unit='L_sol/pc^3')
c21 = fits.Column(name='filt', array=all_filts, format='10A')

t = fits.BinTableHDU.from_columns([c1,c21,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20])
t.writeto('../Result_Tables/all_gal_data_'+slope_ext+phys_ext+'.fits',overwrite=True)
...
fname_results = '../Result_PDFs/all_gal_results_'+slope_ext+phys_ext+'.pdf'
with PdfPages(fname_results) as pdf:
    ...
    for i in range(len(names_05)):
        ...
        if all_lograds[i][0] <= np.log10(5):
            ...
            gamp = 0.69
            M_sol = 4.84 
            I_b = 10**((16.49 - M_sol - 21.572)/(-2.5))
            sig_b = 0.34
            r_b = 3.5

            I_nuke = 2**((beta-gam)/alpha)*I_b*(rad_interp_05[i]/r_b)**gam*(1+(rad_interp_05[i]/r_b)**alpha)**((gam-beta)/alpha)
...
# =============================================================================
            ...
            plt.plot(rad_true_05[i], SBs_true_converted_05[i], color='b', linestyle='', marker='o', label='Data')
            plt.plot(rad_true_05[i], SBs_true_converted_05[i], color='0.8', linewidth=2.0,label='Nuker (Lauer+07)')
            for k in range(len(g_heights_05[i])):
                plt.plot(rad_interp_05[:,i],gausses[k],linestyle='--')

            plt.plot(rad_interp_05[:,i], summed_gauss, color='orange', linewidth=2.0, label= 'MGE fit')#, alpha=0.7)    
            plt.legend()
            plt.yscale('log')
            plt.xscale('log')
            plt.ylim(min(SBs_interp_converted_05[:,i])-
...
# =============================================================================
    ...
    for i in range(len(names_17)):
        ...
        if all_lograds[len(names_05)+len(names_95)+i][0] <= np.log10(5):
        
            # let's plot the MGE results 
            plt.figure(dpi=500)
            # add relevant text
            plt.text(0.05, 0.9, names_17[i], transform=plt.gcf().transFigure, size=10)
            plt.text(0.23, 0.9, 'M_v = {:.2f}'.format(lums_17[i]), transform=plt.gcf().transFigure, size=10)
            plt.text(0.43, 0.9, 'Dist = {:.2f} Mpc'.format(dists_17[i]/10**6), 
                     transform=plt.gcf().transFigure, size=10)
            plt.text(0.7,0.9, 'Inner Rad = {:.2f} pc'.format(radii_17[0,i]), 
                     transform=plt.gcf().transFigure, size=10)
            plt.text(0.9,0.02, nsc_text, 
                     transform=plt.gcf().transFigure, size=10)
            
            gausses = [] 
            for k in range(len(g_heights_17[i])):
                gausses.append(gauss(g_heights_17[i][k],g_sigs_17[i][k],rad_17[:,i]))
                
            summed_gauss = np.zeros_like(rad_17[:,i])
            for k in range(len(rad_17[:,i])):
                for j in range(len(g_heights_17[i])):
                    summed_gauss[k] += gausses[j][k]

            for k in range(len(g_heights_17[i])):
                plt.plot(rad_17[:,i],gausses[k])        
            plt.plot(rad_17[:,i], summed_gauss, color='orange',)#, alpha=0.7)    
            
            plt.yscale('log')
            plt.xscale('log')
            plt.ylim(min(summed_gauss)-
                     min(summed_gauss)*0.5,
                     max(summed_gauss)+
                     max(summed_gauss)*0.5)
            plt.ylabel('Surface Brightness [L$_\odot$/pc$^2$]')
            plt.xlabel('Radius ["]')
            ...
            plt.plot(np.log10(radii_17[:,i]),np.log10(dens_profiles_17[:,i]),label='This Work')
            if len(stone_radii[len(names_05)+len(names_95)+i]) != 1:
                plt.plot(np.log10(stone_radii[len(names_05)+len(names_95)+i]),
                         np.log10(stone_densities[len(names_05)+len(names_95)+i]),label='Stone&Metzger2016')
            plt.xlabel('log(Radius [pc])')
            plt.ylabel('log(Density [M$_{\odot}$/pc$^3$])')
            plt.legend()
            plt.xlim(np.min(np.log10(radii_17[:,i]))-0.2,np.max(np.log10(radii_17[:,i]))+0.2)
        
            if scale_mult*all_hst_ps[len(names_05)+len(names_95)+i] < phys_scale:
                rad_lim = phys_scale
            else:
                rad_lim = scale_mult*all_hst_ps[len(names_05)+len(names_95)+i]
        
            plt.fill_between([np.min(np.log10(radii_17[:,i])),np.log10(rad_lim)],
                             np.min(np.log10(dens_profiles_17[:,i])),np.max(np.log10(dens_profiles_17[:,i]))+
                             0.2*np.max(np.log10(dens_profiles_17[:,i])), color='c',alpha=0.4)
        
            plt.text(0.63,0.66, 'Slope = {:.2f}'.format(slopes_17[i]), 
                     transform=plt.gcf().transFigure, size=8)
            plt.text(0.63,0.72, 'log(Dens@5pc) = {:.2f}'.format(np.log10(central_dens_17[i])), 
                     transform=plt.gcf().transFigure, size=8)
            if stone_dens_at_5pc[len(names_05)+len(names_95)+i] != -99:
                plt.text(0.63,0.63, 'Stone Slope = {:.2f}'.format(stone_slopes[len(names_05)+len(names_95)+i]), 
                         transform=plt.gcf().transFigure, size=8)
                plt.text(0.63,0.69, 'Stone log(Dens@5pc) = {:.2f}'.format(np.log10(stone_dens_at_5pc[len(names_05)+len(names_95)+i])), 
                         transform=plt.gcf().transFigure, size=8)
    ...
    for i in range(len(names_22)):
        ...
        if all_lograds[len(names_05)+len(names_95)+len(names_17)+i][0] <= np.log10(5):
        
            # let's plot the MGE results 
            plt.figure(dpi=500)
            # add relevant text
            plt.text(0.05, 0.9, names_22[i], transform=plt.gcf().transFigure, size=10)
            plt.text(0.23, 0.9, 'M_v = {:.2f}'.format(lums_22[i]), transform=plt.gcf().transFigure, size=10)
            plt.text(0.43, 0.9, 'Dist = {:.2f} Mpc'.format(dists_22[i]/10**6), 
                     transform=plt.gcf().transFigure, size=10)
            plt.text(0.7,0.9, 'Inner Rad = {:.2f} pc'.format(radii_22[0,i]), 
                     transform=plt.gcf().transFigure, size=10)
            plt.text(0.9,0.02, nsc_text, 
                     transform=plt.gcf().transFigure, size=10)
            
            gausses = [] 
            for k in range(len(g_heights_22[i])):
                gausses.append(gauss(g_heights_22[i][k],g_sigs_22[i][k],rad_22[:,i]))
                
            summed_gauss = np.zeros_like(rad_22[:,i])
            for k in range(len(rad_22[:,i])):
                for j in range(len(g_heights_22[i])):
                    summed_gauss[k] += gausses[j][k]

            for k in range(len(g_heights_22[i])):
                plt.plot(rad_22[:,i],gausses[k])  
            plt.plot(rad_22[:,i], summed_gauss, color='orange',)#, alpha=0.7)    
            plt.plot(rad_22[:,i], SBs_22[:,i], color='b', linestyle='', marker='o')
            ...
            plt.plot(np.log10(radii_22[:,i]),np.log10(dens_profiles_22[:,i]),label='This Work')
            if len(stone_radii[len(names_05)+len(names_95)+len(names_17)+i]) != 1:
                plt.plot(np.log10(stone_radii[len(names_05)+len(names_95)+len(names_17)+i]),
                         np.log10(stone_densities[len(names_05)+len(names_95)+len(names_17)+i]),label='Stone&Metzger2016')
            plt.xlabel('log(Radius [pc])')
            plt.ylabel('log(Density [M$_{\odot}$/pc$^3$])')
            plt.legend()
            plt.xlim(np.min(np.log10(radii_22[:,i]))-0.2,np.max(np.log10(radii_22[:,i]))+0.2)
        
            if scale_mult*all_hst_ps[len(names_05)+len(names_95)+len(names_17)+i] < phys_scale:
                rad_lim = phys_scale
            else:
                rad_lim = scale_mult*all_hst_ps[len(names_05)+len(names_95)+len(names_17)+i]
        
            plt.fill_between([np.min(np.log10(radii_22[:,i])),np.log10(rad_lim)],
                             np.min(np.log10(dens_profiles_22[:,i])),np.max(np.log10(dens_profiles_22[:,i]))+
                             0.2*np.max(np.log10(dens_profiles_22[:,i])), color='c',alpha=0.4)
        
            plt.text(0.63,0.66, 'Slope = {:.2f}'.format(slopes_22[i]), 
                     transform=plt.gcf().transFigure, size=8)
            plt.text(0.63,0.72, 'log(Dens@5pc) = {:.2f}'.format(np.log10(central_dens_22[i])), 
                     transform=plt.gcf().transFigure, size=8)
            if stone_dens_at_5pc[len(names_05)+len(names_95)+len(names_17)+i] != -99:
                plt.text(0.63,0.63, 'Stone Slope = {:.2f}'.format(stone_slopes[len(names_05)+len(names_95)+len(names_17)+i]), 
                         transform=plt.gcf().transFigure, size=8)
                plt.text(0.63,0.69, 'Stone log(Dens@5pc) = {:.2f}'.format(np.log10(stone_dens_at_5pc[len(names_05)+len(names_95)+len(names_17)+i])), 
                         transform=plt.gcf().transFigure, size=8)
