...
def get_density_profile(gal_name, max_rad):
    ...
    data4 = hdul[1].data 
    hdul.close()
    abell_notes = np.core.defchararray.find(data4['Note'],'A')
    cross_inds = np.where(abell_notes == 0)
    lauer_names = data4['name'][cross_inds]
    abell_names = data4['Note'][cross_inds]
    ...
    gal_data_w_reff = data3[np.where(data3['name']==gal_name)]
...
# =============================================================================
    M_sol = 4.12 # Vega system - all obs in 'F814W'
# =============================================================================
...
# ======================== DEPROJECT SB PROFILE================================
    ...
    # specify distances and V-Mags for 8 galaxies in Laine03 but not in Lauer07
    #missing_names = np.array(['A3744-M1', 'A1308-M1', 'A0160-M1', 'A2593-M1', 
    #                       'A0147-M1', 'A3559-M1', 'A3367-M1', 'A1836-M1'])
    #dists = np.array([136.58,172.42,153.53,150.75,151.63,,,])
    #vmags = np.array([-22.5,-23.08,-22.82,-23.39,-22.24,,,])
    #print(gal_name)
    #print(gal_data_w_reff)
    #pdb.set_trace()
    #if len(gal_data_w_reff ) == 0:
    #    distance = dists[np.where(missing_names == gal_name)]
...
# =============================================================================
    ...
    return densities, radii, vmag, M_L, ML_type, distance, RA, DEC, SB_data[1],\
        SB_data[0], SBs_i, rad, heights, sigs, SBs
