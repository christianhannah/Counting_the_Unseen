...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  7 14:03:25 2022
...
samps = np.concatenate([np.arange(25,525,25),np.arange(600,2000,100)])
t_slopes = np.zeros(len(samps))
m_slopes = np.zeros(len(samps))
...
#%%    
    ...
    real_slope, real_density = u.get_density_and_slope(np.log10(rad),np.log10(SBs))
    mge_slope, mge_density = u.get_density_and_slope(np.log10(rad),np.log10(summed_gauss))
    ...
    t_slopes[p] = real_slope
    m_slopes[p] = mge_slope
  
#%%
plt.figure(dpi=500)
plt.plot(samps,t_slopes,label='Data Slopes')
plt.plot(samps,m_slopes,linestyle='',marker='.',label='MGE Slopes')
plt.xlabel('Sampling (# of Points)')
plt.ylabel('Power-Law Slope of Profile')
#plt.xlim(0,500)
plt.legend()
