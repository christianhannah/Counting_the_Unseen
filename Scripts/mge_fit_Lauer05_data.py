...
#%%
    ...
    SB_table_filename = '../Data_Sets/Lauer2005_data/SBs_Lauer_2005.fit'
...
# ======================== REPARE DATA FOR FIT ================================
    ...
    #distance1 = gal_data['Dist'][0] * 10**6 # in pc

    # let's get the distance from David's table to use instead
    david_dat = u.get_David_gal_data(gal_name, gal_data['_RA'][0], gal_data['_DE'][0])
...
# =============================================================================
    ...
    SBs_true = 10**(((SB_data['SuBr'] - A_v) - M_sol - 21.572)/(-2.5))
    ...
    rad_new = np.linspace(init_rad[0],init_rad[-1],100)
    I_nuke = 2**((beta-gam)/alpha)*I_b*(rad_new/r_b)**gam*(1+(rad_new/r_b)**alpha)**((gam-beta)/alpha)


# =============================================================================
#     plt.figure(dpi=500)
#     plt.plot(init_rad, SBs_true)
#     plt.plot(rad_new,I_nuke)
#     plt.xscale('log')
#     plt.yscale('log')
#     plt.show()
#     pdb.set_trace()
...
# =============================================================================
    ...
    #M_L = 11 # temporary
    #vmag = gal_data['VMAG'][0] # now from david's table
...
# =============================================================================
    ...
    return densities, radii, vmag, M_L, ML_type, distance, RA, DEC, SB_data['SuBr']-A_v,\
        SB_data['MajAxis'], SBs_i, rad, heights, sigs, SBs, SBs_true, np.array(all_MLs), \
        np.array(all_ML_types), gicolor, dist_flag, summed_gauss, SB_data['Filt'][0]
        

#a = get_density_profile('NGC','4552',100)
